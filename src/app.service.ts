  ...
  public async start() {
    const _ = await this.api.rpc.chain.subscribeFinalizedHeads(
      async (lastHeader) => {
        this.logger.debug(
          `[${this.queueService
            .getProducerQueueType(this.configService.get('QUEUE_NAME'))
            .toLocaleUpperCase()} | ${this.configService.get('QUEUE_NAME')}] ${
            this.chainInfo.chainId
          }_${this.chainInfo.version} ---> Processing block ${lastHeader.hash
            .toString()
            .substring(55)}`,
        );

        const data = await this.parseBlock(lastHeader.hash.toString());
        await this.queueService.send(this.configService.get('QUEUE_NAME'), {
          id: data.id,
          body: data,
        });
      },
